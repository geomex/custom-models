...
This example uses the requests library which you can install with: pip
    install requests We highly recommend that you update SSL
    certificates with: pip install -U urllib3[secure] certifi
...
API_URL = 'https://cfds-ccm-prod.orm.datarobot.com/predApi/'\
    + 'v1.0/deployments/{deployment_id}/predictionsUnstructured'    # noqa
API_KEY = 'NjEzNTI2ZTllMmE5MjZkYjViYzc1NmY3OlZTRStIb296WHNMcFhp'\
    + 'VTlXM1R3VnpMVFBqTm5FL0h6SkFMaExYNC9nL2M9'

DATAROBOT_KEY = '544ec55f-61bf-f6ee-0caf-15c7f919a45d'
DEPLOYMENT_ID = '613e7e39257f6effa39bb924'
...
def make_datarobot_deployment_unstructured_predictions(
    ...
    """Make unstructured predictions on data provided using DataRobot
    deployment_id provided.  See docs for details:
...
def _raise_dataroboterror_for_status(response):
    """Raise DataRobotPredictionError if the request fails along with the 
    response returned"""
    try:
...
def main(filename, deployment_id, mimetype, charset):
    """Return an exit code on script completion or error. Codes > 0 are
    errors to the shell.  Also useful as a usage demonstration of
    `make_datarobot_deployment_unstructured_predictions(data,
    deployment_id, mimetype, charset)`
